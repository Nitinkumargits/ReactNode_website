    ...
    stages {
        ...
        stage('Deploy to EC2') {
            steps {
                ...
                script {
                    sh '''
                        ssh -o StrictHostKeyChecking=no -i $EC2_KEY $EC2_HOST "docker pull $DOCKER_IMAGE:$DOCKER_TAG && docker stop app || true && docker rm app || true && docker run -d  -p 3000:3080 -p 50000:50000 $DOCKER_IMAGE:$DOCKER_TAG"
                    '''
                }
            }
        }
